...
def defacto_rank_api():
    for agent in ['foreigner', 'institution', 'total']:
        if agent == 'total':
            score_qs = ScoreData.objects.all()
        else:
            score_qs = ScoreData.objects.filter(lead_agent=agent)
        ...
        ticker_list = [x[0] for x in score_data]
        ...
        if agent in ['foreigner', 'institution']:
            ...
            print(score_rank)
            start=time.time()
            score_rank_list = score_rank.values.tolist()
            score_list = []
            ...
            print(rank_change)
            start=time.time()
            rank_change_list = rank_change.values.tolist()
            rank_list = []
